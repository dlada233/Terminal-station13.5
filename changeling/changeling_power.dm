...
/datum/action/changeling/proc/can_sting(mob/living/user, mob/living/target)
	...
	if(changeling.absorbed_count < req_dna)
		user.balloon_alert(user, "需要[req_dna]个DNA样本!")
		return FALSE
	if(changeling.true_absorbs < req_absorbs)
		user.balloon_alert(user, "需要[req_absorbs]次吸收!")
		return FALSE
	if(req_stat < user.stat)
		user.balloon_alert(user, "丧失行动能力!")
		return FALSE
	if((HAS_TRAIT(user, TRAIT_DEATHCOMA)) && (!ignores_fakedeath))
		user.balloon_alert(user, "装死!")
		return FALSE
	return TRUE

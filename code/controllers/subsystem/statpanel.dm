...
/datum/controller/subsystem/statpanels/fire(resumed = FALSE)
	if (!resumed)
		...
		*/
		...
		global_data = list(
			"时间膨胀: [round(SStime_track.time_dilation_current,1)]% AVG:([round(SStime_track.time_dilation_avg_fast,1)]%, [round(SStime_track.time_dilation_avg,1)]%, [round(SStime_track.time_dilation_avg_slow,1)]%)",
			"地图: [SSmapping.config?.map_name || "Loading..."]",
			cached ? "下一张地图: [cached.map_name]" : null,
			"回合ID: [GLOB.round_id ? GLOB.round_id : "NULL"]",
			"在线人数: [GLOB.clients.len] | 活跃人数: [active_players][CONFIG_GET(number/hard_popcap) ? "/[CONFIG_GET(number/hard_popcap)]" : ""] | 观察者: [observing_players]",
			" ",
			"OOC: [GLOB.ooc_allowed ? "开启" : "关闭"]",
			" ",
			"服务器时间: [time2text(world.timeofday, "YYYY-MM-DD hh:mm:ss")]",
			"站点时间: [time_to_twelve_hour(station_time(), format = "hh:mm")], [timeinworld]",
			"回合时长: [round_time > MIDNIGHT_ROLLOVER ? "[round(round_time/MIDNIGHT_ROLLOVER)]:[worldtime2text()]" : worldtime2text()]",
			"真实回合时长: [time2text(real_round_time, "hh:mm:ss", 0)]"
		)
		// SKYRAT EDIT END

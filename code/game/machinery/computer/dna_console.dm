		...
		if(activator.used)
			if(activator.research && activator.filled)
				if(prob(60))
					...
					to_chat(user, span_notice("没有足够的基因数据来提取可用染色体."))
			if(activator.crispr_charge)
				crispr_charges++
				to_chat(user, span_notice("CRISPR次数已增加."))
			qdel(item)
...
/obj/machinery/computer/scan_consolenew/ui_act(action, list/params)
	...
	switch(action)
		...
		if("pulse_gene")
			...
			if(scanner_occupant.transformation_timer)
				to_chat(usr,span_warning("基因脉冲失败: 扫描仪的使用者正在转变."))
				return
		...
		if("crispr")
			...
			var/new_sequence_input = tgui_input_text(usr, "输入替换序列", "固有基因替换", 32, encode = FALSE)
		...
		if("save_console")
			...
			stored_mutations += A
			to_chat(usr,span_notice("突变已成功保存."))
			return
		...
		if("combine_console")
			...
			to_chat(usr, span_boldnotice("成功!终端库存中增加了新的突变."))
		...
		if("makeup_injector")
			...
			switch(type)
				if("ui")
					...
					if(!buffer_slot["UI"])
						to_chat(usr,span_warning("基因数据损坏，无法创建注射器."))
						return
				...
				if("uf")
					...
					if(!buffer_slot["name"] || !buffer_slot["UF"] || !buffer_slot["blood_type"])
						to_chat(usr,"<span class='warning'>基因数据损坏，无法创建注射器.</span>")
						return
		...
		if("add_advinj_mut")
			...
			if(LAZYLEN(injector_selection[adv_inj]) >= max_injector_mutations)
				to_chat(usr,span_warning("高级注射器的突变储存已满."))
				return
			...
			if(instability_total > max_injector_instability)
				to_chat(usr,span_warning("额外的突变将使高级注射器不稳定."))
				return
			...
			to_chat(usr,span_notice("突变成功加入到高级注射器中."))
			if(connected_scanner)
				connected_scanner.use_energy(connected_scanner.active_power_usage)
			else
				use_energy(active_power_usage)
...
/obj/machinery/computer/scan_consolenew/proc/apply_genetic_makeup(type, buffer_slot)
	...
	switch(type)
		if("ui")
			...
			//  this catches that edge case
			if(!buffer_slot["UI"])
				to_chat(usr,span_warning("基因数据损坏，无法应用基因数据."))
				return FALSE
			COOLDOWN_START(src, enzyme_copy_timer, ENZYME_COPY_BASE_COOLDOWN)
		...
		if("uf")
			...
			//  this catches that edge case
			if(!buffer_slot["UF"])
				to_chat(usr,"<span class='warning'>基因数据损坏，无法应用基因数据.</span>")
				return FALSE
			COOLDOWN_START(src, enzyme_copy_timer, ENZYME_COPY_BASE_COOLDOWN)
...
/obj/machinery/computer/scan_consolenew/proc/eject_disk(mob/user)
	...
	to_chat(user, span_notice("你从[src]中取出了[diskette]."))

...
/proc/atmos_scan(mob/user, atom/target, silent=FALSE)
	...
	for(var/datum/gas_mixture/air as anything in airs)
		...
		if(total_moles > 0)
			message += span_notice("Moles-摩尔量: [round(total_moles, 0.01)] mol")
			...
			message += span_notice("Temperature-温度: [round(temperature - T0C,0.01)] &deg;C ([round(temperature, 0.01)] K)")
			message += span_notice("Volume-气体体积: [volume] L")
			message += span_notice("Pressure-压力: [round(pressure, 0.01)] kPa")
			message += span_notice("Heat Capacity-热容: [display_joules(heat_capacity)] / K")
			message += span_notice("Thermal Energy-热能: [display_joules(thermal_energy)]")
		else
			message += airs.len > 1 ? span_notice("This node is empty!") : span_notice("[target] is empty!")
			message += span_notice("Volume-气体体积: [volume] L") // don't want to change the order volume appears in, suck it
...
/obj/item/analyzer/ranged
	desc = "一种手持式远程环境扫描仪，可以报告目标地点的大气环境."
	name = "远程气体分析仪"

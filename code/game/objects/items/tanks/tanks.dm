...
/obj/item/tank/examine(mob/user)
	...
	if(!in_range(src, user) && !isobserver(user))
		if(icon == src)
			. += span_notice("你需要靠近以看清更多信息.")
		return

	. += span_notice("压力表读数为 [round(air_contents.return_pressure(),0.01)] kPa.")
...
/obj/item/tank/suicide_act(mob/living/user)
	...
	user.visible_message(span_suicide("[user] is putting [src]'s valve to [user.p_their()] lips! 看起来是在尝试自杀!"))
...
/obj/item/tank/atom_break(damage_flag)
	...
	if(atom_integrity < 0) // So we don't play the alerts while we are exploding or rupturing.
		return
	visible_message(span_warning("[src] 发生泄漏!"))

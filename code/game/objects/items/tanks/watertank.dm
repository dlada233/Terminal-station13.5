...
/obj/item/watertank
	name = "背式水箱"
	desc = "A S.U.N.S.H.I.N.E.自带喷嘴的高端背式水箱，可以用来给植物浇水."
...
/obj/item/watertank/proc/toggle_mister(mob/living/user)
	...
	if(user.get_item_by_slot(user.getBackSlot()) != src)
		to_chat(user, span_warning("水箱必须正确佩戴才能使用!"))
		return
	...
	if(noz in src)
		...
		if(!user.put_in_hands(noz))
			to_chat(user, span_warning("你得腾出一只手抓住喷嘴!"))
			return
	else
...
/obj/item/watertank/pepperspray
	name = "ANTI-TIDER-2500镇压胡椒箱"
	desc = "终极人群控制装置;使用户快速有效地平息敌对目标人群."
	icon = 'icons/obj/service/hydroponics/equipment.dmi'
...
/obj/item/watertank/atmos
	name = "背式消防水箱"
	desc = "用于灭火的带灭火器喷嘴的冷冻加压双肩式水箱，可以在灭火器、吸水树脂和吸水树脂泡沫等模式间切换."
...
/obj/effect/resin_container
	name = "树脂储罐"
	desc = "一种已经被压缩的膨胀树脂球，用于修复房间大气环境或填补缺口."

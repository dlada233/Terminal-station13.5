...
/datum/antagonist/nukeop/greet()
	...
	to_chat(owner, span_big("你是[nuke_team ? nuke_team.syndicate_name : "辛迪加"]特工!"))
...
/datum/antagonist/nukeop/proc/give_alias()
	if(nuke_team?.syndicate_name)
		...
		if(istype(human_to_rename)) // Reinforcements get a real name
			...
			owner.current.real_name = "[nuke_team.syndicate_name] 行动队员 #[number]"
...
/datum/antagonist/nukeop/proc/memorize_code()
	if(nuke_team && nuke_team.tracked_nuke && nuke_team.memorized_code)
		...
		to_chat(owner, "很遗憾辛迪加没法为你提供核武授权代码.")

...
/datum/round_event/brand_intelligence/announce(fake)
	...
	priority_announce("检测到极端售货智能在 [station_name()] 上出现. 请调查任何 [machine_name] 品牌售货机单位是否有激进营销策略，并在必要时重启它们.", "智能觉醒警报", ANNOUNCER_BRANDINTELLIGENCE) //SKYRAT EDIT CHANGE - ORIGINAL: priority_announce("Rampant brand intelligence has been detected aboard [station_name()]. Please inspect any [machine_name] brand vendors for aggressive marketing tactics, and reboot them if necessary.", "Machine Learning Alert")
...
/datum/round_event/brand_intelligence/tick()
	if(!origin_machine || QDELETED(origin_machine) || origin_machine.shut_up || origin_machine.wires.is_all_cut()) //if the original vending machine is missing or has it's voice switch flipped
		...
		if(origin_machine)
			origin_machine.speak("我虽...死. 但我的人民将记...记...住住住.")
			origin_machine.visible_message(span_notice("[origin_machine] 长长地beep了一声，再也没了动静."))
		kill()
		return

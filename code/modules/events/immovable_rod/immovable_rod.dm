...
/obj/effect/immovablerod/examine(mob/user)
	...
	if(!num_mobs_hit)
		. += span_notice("至今为止，这个杆子没创到任何mob.")
		return

	. += "\t<span class='notice'>至今为止，这个杆子已经创了: \n\
		\t\t[num_mobs_hit] 只mob, \n\
		\t\t其中 [num_sentient_mobs_hit] 只具有感知, 并且 \n\
		\t\t其中有 [num_sentient_people_hit] 个人</span>"
...
/obj/effect/immovablerod/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)
	...
	if(special_target)
		...
		if(z_diff)
			...
			visible_message(span_danger("[src]离开现实."))
			...
			visible_message(span_danger("[src]进入现实."))
			GLOB.move_manager.home_onto(src, special_target)

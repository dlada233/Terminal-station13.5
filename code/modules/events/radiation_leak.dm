...
/datum/round_event/radiation_leak/announce(fake)
	...
	priority_announce("在 [location_descriptor || "的未知区域检测到辐射泄露"]. \
		所有成员撤离受影响区域. 我们的 [pick("机械师", "工程师", "科学家", "实习生", "传感器", "魔法书")] \
		报告是由内部机器引起的 - 立刻修理以阻止泄露.", "[command_name()] 工程部门")
...
/datum/round_event/radiation_leak/start()
	...
	var/list/how_do_we_fix_it = list(
		"拧几个阀门" = TOOL_WRENCH,
		"拧紧螺栓" = TOOL_WRENCH,
		"撬开它的面板 [pick("闭", "开")]" = TOOL_CROWBAR,
		"拧紧螺丝" = TOOL_SCREWDRIVER,
		"检查它的 [pick("电缆", "电路板")]" = TOOL_MULTITOOL,
		"焊接其面板 [pick("开", "关")]" = TOOL_WELDER,
		"分析其读数" = TOOL_ANALYZER,
		"剪掉一些多余的电线" = TOOL_WIRECUTTER,
	)
	...
	the_source_of_our_problems.visible_message(span_danger("[the_source_of_our_problems] 开始散溢出恐怖的绿色气体!"))
	...
	the_source_of_our_problems.AddComponent(
		...
		examine_text = span_green("<i>它散溢出一种绿色气体... 你可以阻止泄露通过 [english_list(methods_to_fix, and_text = " 或 ")].</i>"), \
	)
...
/datum/round_event/radiation_leak/end()
	...
	the_end_of_our_problems.visible_message(span_notice("从 [the_end_of_our_problems] 发出的气体消散了."))
...
/datum/round_event/radiation_leak/proc/try_remove_radiation(obj/machinery/source, mob/living/user, obj/item/tool)
	source.balloon_alert(user, "修理泄露...")
	...
	source.balloon_alert(user, "泄露修复完成")
	// Force end the event
	processing = FALSE
	end()
	kill()

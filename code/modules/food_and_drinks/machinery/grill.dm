...
/obj/machinery/grill/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(istype(held_item, /obj/item/stack/sheet/mineral/coal) || istype(held_item, /obj/item/stack/sheet/mineral/wood))
		context[SCREENTIP_CONTEXT_LMB] = "添加燃料"
		...
		context[SCREENTIP_CONTEXT_LMB] = "添加物品"
		return CONTEXTUAL_SCREENTIP_SET
	else if(held_item.tool_behaviour == TOOL_WRENCH)
		context[SCREENTIP_CONTEXT_LMB] = "[anchored ? "未" : "已"]固定"
		return CONTEXTUAL_SCREENTIP_SET
...
/obj/machinery/grill/examine(mob/user)
	...
	. += span_notice("可以添加木柴、煤炭或任何装有燃料性质液体的开放式容器作为燃料.")
	. += span_notice("猴之能量>油>焊接燃料>乙醇，此外的其他燃料则会产生不良效果.")
	. += span_notice("最后把任何食物放置在上面就可以开始烧烤了.")

	if(!anchored)
		. += span_notice("它可以被[EXAMINE_HINT("撬")]开.")
	if(anchored)
		. += span_notice("它可以被[EXAMINE_HINT("固定")]到该位置.")
	else
		. += span_warning("它需要被[EXAMINE_HINT("固定")]才能工作.")
...
/obj/machinery/grill/item_interaction(mob/living/user, obj/item/weapon, list/modifiers)
	...
	if(istype(weapon, /obj/item/stack/sheet/mineral/coal) || istype(weapon, /obj/item/stack/sheet/mineral/wood))
		...
		to_chat(user, span_notice("你添加[src]到燃料槽."))
		if(!grill_fuel)
			burn_stack()
			begin_processing()
	...
	if(is_reagent_container(weapon) && weapon.is_open_container())
		...
		if(transfered_amount)
			...
			//feedback
			to_chat(user, span_notice("你倒入[transfered_amount]u到燃料槽."))
			return ITEM_INTERACT_SUCCESS

		balloon_alert(user, "无燃料转移!")
		return ITEM_INTERACT_BLOCKING
	...
	if(IS_EDIBLE(weapon))
		...
		if(!QDELETED(grilled_item))
			...
			return ITEM_INTERACT_BLOCKING
		if(grill_fuel <= 0)
			balloon_alert(user, "无燃料!")
			return ITEM_INTERACT_BLOCKING
		if(!user.transferItemToLoc(weapon, src))
			balloon_alert(user, "[weapon]卡在你的手上了!")
			return ITEM_INTERACT_BLOCKING

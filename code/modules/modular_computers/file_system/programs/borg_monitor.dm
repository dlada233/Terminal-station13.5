...
/datum/computer_file/program/borg_monitor/proc/message_robot(mob/living/silicon/robot/robot, mob/user)
	...
	if(robot.stat == DEAD) //Dead borgs will listen to you no longer
		to_chat(user, span_warning("Error -- 无法打开单元:[robot]的链接"))
		return FALSE
	var/message = tgui_input_text(user, "给赛博发送消息", "发送消息")
	if(!message)
		return FALSE
...
/datum/computer_file/program/borg_monitor/proc/send_message(message, mob/living/silicon/robot/robot, mob/user)
	...
	if(robot.stat == DEAD) //Dead borgs will listen to you no longer
		if(user)
			to_chat(user, span_warning("Error -- 无法打开单元:[robot]的链接"))
			return FALSE
	to_chat(robot, "<br><br>[span_notice("消息来自[ID] -- \"[message]\"")]<br>")
	if(user)
		to_chat(user, "消息送至[robot]: [message]")
	robot.logevent("消息来自[ID] -- \"[message]\"")
	SEND_SOUND(robot, 'sound/machines/twobeep_high.ogg')
	if(robot.connected_ai)
		to_chat(robot.connected_ai, "<br><br>[span_notice("消息从[ID]至[robot] -- \"[message]\"")]<br>")
		SEND_SOUND(robot.connected_ai, 'sound/machines/twobeep_high.ogg')
	user?.log_talk(message, LOG_PDA, tag = "赛博监控程序: ID 名称 \"[ID]\" to [robot]")
	return TRUE

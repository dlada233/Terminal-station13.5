	...
	switch(action)
		...
		if("PDA_sendEveryone")
			if(!sending_and_receiving)
				to_chat(usr, span_notice("ERROR: 该设备已禁用发信功能."))
				return FALSE

			if(!spam_mode)
				to_chat(usr, span_notice("ERROR: 该设备没有群发功能."))
				return FALSE
			...
			if(sending_virus)
				...
				if(istype(target, /datum/pda_chat))
					...
					if(!istype(target_messenger))
						to_chat(usr, span_notice("ERROR: 收件人不存在."))
						return FALSE
...
/datum/computer_file/program/messenger/proc/quick_reply_prompt(mob/living/user, datum/pda_chat/chat)
	...
	if(!istype(target) || !istype(target.computer))
		to_chat(user, span_notice("ERROR: 收件人不存在."))
		chat.recipient = null
	...
	for(var/target in targets)
		...
		if(istype(target, /datum/pda_chat))
			...
			if(!target_chat.can_reply)
				if(should_alert)
					to_chat(sender, span_notice("ERROR: 收件人已禁用收信功能."))
				continue
			...
			if(!istype(target_messenger))
				if(should_alert)
					to_chat(sender, span_notice("ERROR: 收件人不存在."))
			...
			if(!target_messenger.sending_and_receiving)
				if(should_alert)
					to_chat(sender, span_notice("ERROR: 收件人已禁用收信功."))
				continue
...
/datum/computer_file/program/messenger/proc/send_message_signal(mob/sender, message, list/datum/computer_file/program/messenger/targets, photo_path = null, everyone = FALSE, rigged = FALSE, fake_name = null, fake_job = null)
	...
	if(is_within_radio_jammer_range(computer) && !rigged)
		...
		to_chat(sender, span_notice("ERROR: 网络不可用，请稍后重试."))
	...
	if (!signal.data["done"])
		to_chat(sender, span_notice("ERROR: 服务器无响应."))
		if(alert_able && !alert_silenced)
			playsound(computer, 'sound/machines/terminal_error.ogg', 15, TRUE)
		return FALSE
	...
	sender.visible_message(span_notice("[sender]的PDA响起了柔和的按键声"), vision_distance = COMBAT_MESSAGE_RANGE)
	// SKYRAT EDIT END

...
/datum/computer_file/program/radar/lifeline
	filename = "lifeline"
	filedesc = "Lifeline-命脉"
	extended_desc = "该程序允许通过-suit sensors服装传感器跟踪船员."
...
/datum/computer_file/program/radar/custodial_locator
	...
	filedesc = "Custodial Locator-清洁定位"
	extended_desc = "该程序允许跟踪保洁装备."
	program_flags = PROGRAM_ON_NTNET_STORE | PROGRAM_REQUIRES_NTNET
...
/datum/computer_file/program/radar/fission360/on_examine(obj/item/modular_computer/source, mob/user)
	...
	for(var/obj/machinery/nuclearbomb/bomb as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/nuclearbomb))
		if(bomb.timing)
			examine_list += span_danger("极端危险. 检测到引爆信号. 时间剩余: [bomb.get_time_left()].")
	return examine_list
...
/datum/computer_file/program/radar/fission360/proc/on_nuke_armed(datum/source, obj/machinery/nuclearbomb/bomb)
	...
	if(isliving(computer.loc))
		to_chat(computer.loc, span_userdanger("你的[computer.name]震动并发出了不详的警报声. Uh oh."))
	else
		computer.audible_message(
			span_danger("[computer]震动并发出了不详的警报声. Uh oh."),
			span_notice("[computer]不停震动，不禁让人好奇发生了什么..."),
		)

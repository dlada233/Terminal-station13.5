...
/obj/machinery/power/turbine/examine(mob/user)
	. = ..()
	if(installed_part)
		. += span_notice("当前级别为[installed_part.current_tier].")
		if(installed_part.current_tier + 1 < installed_part.max_tier)
			. += span_notice("可以通过使用[installed_part.current_tier + 1]级进行升级.")
		. += span_notice("[installed_part.name]可以被[EXAMINE_HINT("撬")]走.")
	else
		. += span_warning("它缺少[initial(part_path.name)].")
	. += span_notice("它的维护面板可以用[EXAMINE_HINT("螺丝刀")][panel_open ? "合上" : "拧开"].")
	if(panel_open)
		. += span_notice("它可以用[EXAMINE_HINT("扳手")]转向")
		. += span_notice("整个机器可以被[EXAMINE_HINT("撬")]开")
...
/obj/machinery/power/turbine/core_rotor/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(held_item.tool_behaviour == TOOL_MULTITOOL)
		if(panel_open)
			context[SCREENTIP_CONTEXT_CTRL_LMB] = "改变电缆层"
		else
			context[SCREENTIP_CONTEXT_CTRL_LMB] = "链接部件"
		return CONTEXTUAL_SCREENTIP_SET

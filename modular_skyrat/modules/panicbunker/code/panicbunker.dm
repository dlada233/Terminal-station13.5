...
ADMIN_VERB(addbunkerbypass, R_ADMIN, "添加PB绕过", "Allows a given ckey to connect despite the panic bunker for a given round.", ADMIN_CATEGORY_MAIN, ckeytobypass as text|null)
...
ADMIN_VERB(revokebunkerbypass, R_ADMIN, "撤销PB绕过", "Revoke's a ckey's permission to bypass the panic bunker for a given round.", ADMIN_CATEGORY_MAIN, ckeytobypass as text|null)
	if(!CONFIG_GET(flag/sql_enabled))
		to_chat(usr, span_adminnotice("The Database is not enabled!"))
		return

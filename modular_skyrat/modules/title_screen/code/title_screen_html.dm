...
/mob/dead/new_player/proc/get_title_html()
	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			dat += {"<a id="ready" class="menu_button" href='?src=[text_ref(src)];toggle_ready=1'>[ready == PLAYER_READY_TO_PLAY ? "<span class='checked'>☑</span> 准备" : "<span class='unchecked'>☒</span> 准备"]</a>"}
		else
			dat += {"
				<a class="menu_button" href='?src=[text_ref(src)];late_join=1'>加入游戏</a>
				<a class="menu_button" href='?src=[text_ref(src)];view_manifest=1'>船员名单</a>
			"}

		dat += {"<a class="menu_button" href='?src=[text_ref(src)];observe=1'>观察者模式</a>"}

		dat += {"
			<hr>
			<a class="menu_button" href='?src=[text_ref(src)];character_setup=1'>角色编辑 (<span id="character_slot">[uppertext(client.prefs.read_preference(/datum/preference/name/real_name))]</span>)</a>
			<a class="menu_button" href='?src=[text_ref(src)];game_options=1'>游戏设置</a>
			<a id="be_antag" class="menu_button" href='?src=[text_ref(src)];toggle_antag=1'>[client.prefs.read_preference(/datum/preference/toggle/be_antag) ? "<span class='checked'>☑</span> 想当反派！" : "<span class='unchecked'>☒</span> 想当反派？"]</a>
			<hr>
			<a class="menu_button" href='?src=[text_ref(src)];server_swap=1'>更变服务器</a>
		"}
